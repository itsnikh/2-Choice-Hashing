...
#include "hash.c"	/* Here, this header file will include the Hash function which can also be found in src folder */

void clearscreen(void)	/* A function to clear the Bash screen, (may/ may not work on every computer) */
{
	printf("\033[H\033[J");
...
/* Monitors here for any incoming packet(s) */
		...
		if(FD_ISSET(tcp,&fds))
		{
			...
			if( ((strcmp((char*)buf,"exit"))==0) || ((strcmp((char*)buf,"EXIT"))==0) )  // ((cmp = strcmp(temp->ip,ip))==0)
			{
				printf("\n\nClosing the Server!!\n\n");
				sleep(2);
				close(cfd);
